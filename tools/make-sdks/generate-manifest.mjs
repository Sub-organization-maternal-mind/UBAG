...
`;
  const goPath = resolve(ROOT, "packages/sdk-go/generated_contract_manifest.go");
  mkdirSync(dirname(goPath), { recursive: true });
  writeFileSync(goPath, out);
}
...
function writeRust(m) {
  const endpoints = Object.entries(m.endpoints)
    ...
    .join("\n");
  const codes = Object.entries(m.errorCodes)
    .map(([k, v]) => `        (${JSON.stringify(k)}, ManifestErrorCode { category: ${JSON.stringify(v.category)}, retryable: ${v.retryable}, retry_after_ms: ${v.retry_after_ms ?? 0} }),`)
    .join("\n");
  const out = `// Generated by tools/make-sdks/generate-manifest.mjs. Do not edit by hand.
