...
## 2026-07-16 Fix: multi-turn conversation recall (turn-aware response read)

Live testing of conversation affinity found the resume path bound correctly (chat URL stored) but the **second turn returned the first turn's answer** — e.g. establish "the word is BANANA", then ask "what was the word?" and get the prior turn's "OK". Root cause in `apps/worker/ubag_worker/live/page_driver.py`: the response reader used `_first_visible` → `locator(sel).first`, but `response_container` matches **every** assistant turn (e.g. `div[data-message-author-role='assistant']`). On a resumed multi-turn thread `.first` is the OLDEST turn's answer — already rendered and stable, so streaming settled instantly on it. Single-turn jobs only worked because a fresh chat has exactly one assistant node. DeepSeek's `final_answer_container` had the same `.first`-across-turns flaw.

Fix (turn-aware read, `PlaywrightPageDriver` only — mock is scripted and unaffected): `submit_prompt` snapshots per-candidate `response_container` counts **before** submitting (baseline of prior turns); `stream_response` now waits (paced poll, no busy-spin) until a candidate's count exceeds its baseline — i.e. THIS turn's node has appeared — then binds `.last` (newest); `read_final_response` reads `.last` via a new `_newest_visible`. On a fresh chat every baseline is 0 and `.last == .first`, so the single-turn path is byte-identical. Times out into `DriftDetectedError` (fail loud) rather than ever returning a prior turn's answer. Covered by `apps/worker/tests/test_conversation_turn_read.py` (fake multi-turn Playwright page; 5 tests incl. deferred-reveal + single-turn regression). Full worker suite green (362).
