...
v0 starts with app-secret authentication. Short-lived RS256 app JWTs are available on the HTTP gateway: set `UBAG_APP_JWT_PUBLIC_KEY` (inline PEM, literal `\n` accepted) or `UBAG_APP_JWT_PUBLIC_KEY_FILE` (PEM file) and clients present `Authorization: Bearer <jwt>` carrying `tid` (tenant), `sub` (app id), `role` (e.g. `service`), and a required `exp`. Each downstream client app then gets its own isolated `(tenant, app)` scope for jobs, conversations, rate limits, and listings. Tokens with empty identity claims or no expiry are rejected. Personal Access Tokens (PATs) are also available on the HTTP gateway: set `UBAG_PAT_ENABLED=true` and a superadmin issues opaque `ubag_pat_…` bearer tokens via `POST /v1/auth/pat`, each scoped to a `(tenant, app, role)`. Only the SHA-256 hash of each token is persisted, and tokens survive restarts when the gateway store is SQLite or Postgres (the in-memory store loses them on restart). Use App JWTs for short-lived, statelessly-verified client identity and PATs for long-lived issued credentials. Device tokens and OIDC follow in v1; mTLS is a standard/enterprise path; gRPC remains app-secret-only.
