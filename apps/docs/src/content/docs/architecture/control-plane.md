...
- Template lookup/application is wired into job creation before payload validation, idempotency hashing, storage, and executor enqueue. The built-in catalog is available through `/v1/templates`, scoped template reads, and Pongo2-compatible render dry-runs.

## v1 expansion

- Batch jobs, richer workflow/cache execution, durable template management, and live-provider operational hardening for the route surfaces already present in v0.
