...
// budget the caller opted into). The lane token is freed via the same
// tuple-based ConcurrencyRegistry.Release the worker consumer uses on a normal
// terminal transition, and UpdateStatus commits the terminal transition exactly
// once, so a reap that races a real terminal transition can never double-count
// the lane.
	...
	for _, status := range jobstore.LifecycleStatuses() {
		...
		for _, job := range jobs {
			...
			if rp.Concurrency != nil {
				// Release the same (tenant, target, identity) lane token the worker
				// consumer frees on a normal terminal transition (workerconsumer.go),
				// so a reaped stuck job returns its in-flight slot. UpdateStatus above
				// already committed the terminal transition exactly once, so this
				// release runs at most once per job and cannot double-count the lane.
				rp.Concurrency.Release(updated.TenantID, updated.Target, updated.AppID)
			}
			if rp.Notifier != nil {
