<script lang="ts">
  ...
  import type { App, ListResponse } from '$lib/api/types';
  ...
  async function load() {
    ...
    error = null;
    denied = false;
    const res = await api.get<ListResponse<App>>('/v1/apps');
    loading = false;
    if (res.denied) { denied = true; return; }
    if (res.error) { error = res.error; return; }
    const data = res.data as Record<string, unknown> | null;
    items = (data?.['items'] ?? data?.['apps'] ?? []) as App[];
  }
        ...
        <tbody class="divide-y divide-rule">
          {#each filtered as app (app.id)}
            <tr class="hover:bg-paper-soft transition-colors">
              <td class="px-4 py-2.5 font-mono text-ink-mute text-xs">{app.id.slice(0, 8)}…</td>
              <td class="px-4 py-2.5 text-ink font-medium">{app.name}</td>
              <td class="px-4 py-2.5 text-ink-soft font-mono text-xs">{app.version ?? '—'}</td>
              <td class="px-4 py-2.5"><StatusBadge status={app.status ?? 'unknown'} /></td>
            </tr>
          {/each}
        </tbody>
      </table>
