...
_PROVIDER_ID = "deepseek_web"
_SCAFFOLD_MESSAGE = (
    "DeepSeek Web adapter is a safe-mode scaffold only. It requires a "
...
class DeepSeekWebAdapter:
    """DeepSeek Web manual-session adapter."""
...
def build_deepseek_web_events(payload: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    return DeepSeekWebAdapter().run(payload)


def build_deepseek_web_live_events(
    payload: Mapping[str, Any], *, driver: Optional[Any] = None
) -> List[Mapping[str, Any]]:
    return DeepSeekWebAdapter().run_live(payload, driver=driver)
