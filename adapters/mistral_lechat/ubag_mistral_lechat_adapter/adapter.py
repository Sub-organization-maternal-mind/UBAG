...
_PROVIDER_ID = "mistral_lechat"
_SCAFFOLD_MESSAGE = (
    "Mistral Le Chat adapter is a safe-mode scaffold only. It requires a "
...
class MistralLeChatAdapter:
    """Mistral Le Chat manual-session adapter."""
...
def build_mistral_lechat_events(payload: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    return MistralLeChatAdapter().run(payload)


def build_mistral_lechat_live_events(
    payload: Mapping[str, Any], *, driver: Optional[Any] = None
) -> List[Mapping[str, Any]]:
    return MistralLeChatAdapter().run_live(payload, driver=driver)
