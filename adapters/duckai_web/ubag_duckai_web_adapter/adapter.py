...
_PROVIDER_ID = "duckai_web"
_SCAFFOLD_MESSAGE = (
    "Duck.ai Web adapter is a safe-mode scaffold only. It requires a "
...
class DuckaiWebAdapter:
    """Duck.ai Web manual-session adapter."""
...
def build_duckai_web_events(payload: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    return DuckaiWebAdapter().run(payload)


def build_duckai_web_live_events(
    payload: Mapping[str, Any], *, driver: Optional[Any] = None
) -> List[Mapping[str, Any]]:
    return DuckaiWebAdapter().run_live(payload, driver=driver)
