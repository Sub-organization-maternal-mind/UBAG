...
_PROVIDER_ID = "gemini_web"
_SCAFFOLD_MESSAGE = (
    "Gemini Web adapter is a safe-mode scaffold only. It requires a "
...
class GeminiWebAdapter:
    """Gemini Web manual-session adapter."""
...
def build_gemini_web_events(payload: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    return GeminiWebAdapter().run(payload)


def build_gemini_web_live_events(
    payload: Mapping[str, Any], *, driver: Optional[Any] = None
) -> List[Mapping[str, Any]]:
    return GeminiWebAdapter().run_live(payload, driver=driver)
