...
_PROVIDER_ID = "claude_web"
_SCAFFOLD_MESSAGE = (
    "Claude Web adapter is a safe-mode scaffold only. It requires a "
...
class ClaudeWebAdapter:
    """Claude Web manual-session adapter."""
...
def build_claude_web_events(payload: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    return ClaudeWebAdapter().run(payload)


def build_claude_web_live_events(
    payload: Mapping[str, Any], *, driver: Optional[Any] = None
) -> List[Mapping[str, Any]]:
    return ClaudeWebAdapter().run_live(payload, driver=driver)
