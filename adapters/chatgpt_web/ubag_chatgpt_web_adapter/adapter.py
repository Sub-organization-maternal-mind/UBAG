...
_PROVIDER_ID = "chatgpt_web"
_SCAFFOLD_MESSAGE = (
    "ChatGPT Web adapter is a safe-mode scaffold only. It requires a "
...
class ChatGptWebAdapter:
    """ChatGPT Web manual-session adapter."""
...
def build_chatgpt_web_events(payload: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    return ChatGptWebAdapter().run(payload)


def build_chatgpt_web_live_events(
    payload: Mapping[str, Any], *, driver: Optional[Any] = None
) -> List[Mapping[str, Any]]:
    return ChatGptWebAdapter().run_live(payload, driver=driver)
