# Universal Browser-Automation Gateway — World-Class Engineering Blueprint v2.1
...
**Supersedes:** *UBAG World-Class Engineering Blueprint v2.0* (2026-05-22)
...
### 0.1 What changed in v2.1 (this revision)

v2.1 keeps every v2.0 decision and adds one headline capability plus a sweep of hardening and cross-compatibility deltas.

| Area | v2.0 | v2.1 |
|---|---|---|
| **Concurrency model** | "One job per session"; parallelism only via separate browser *contexts* (§12.6). Tabs barely modeled. | Explicit **Browser → Provider Context → Channel Tab** hierarchy. One browser hosts **many logged-in providers at once**; each provider **auto-fans-out into parallel tabs** on demand. (§12.6–§12.13) |
| **Many providers at once** | Implicit (one context = one chat) | **Multi-provider browser**: tab group per provider, isolated cookies/fingerprint/proxy, one shared process. (§12.7) |
| **Same provider, parallel** | Not addressed | **Channel-pool** per (tenant, provider, identity): "more than one request ⇒ open another tab and run it in parallel," with warm pools and auto-scale. (§12.8) |
| **Anti-ban safety** | Stealth only | **Adaptive concurrency (AIMD)** + **global submit pacer** + **conversation-model detection** so parallel tabs never get an account flagged. (§12.9) |
| **Shared-login races** | Not addressed | **Single-flight re-login**, per-tab conversation isolation, `storage_state` snapshots. (§12.10) |
| **Failure isolation** | Generic retries | **Blast-radius-by-level** (tab / context / browser / worker) nested bulkheads with automatic recovery. (§12.12) |
| **Cross-engine** | Chromium + Patchright only | Pluggable engine: Chromium (CDP), **Firefox**, **WebKit**, and **WebDriver BiDi** (W3C standard); **remote browser grids** (browserless / Selenium Grid 4 / Playwright server). (§13.10–§13.12) |
| **Schema** | sessions table | + `browser_instances`, `provider_contexts`, `browser_tabs`; `browser_sessions` retained as a compatibility alias. (§22) |

If you only read one new thing, read **§12.6–§12.13** — that is the multi-tab orchestration engine.
...
**Error code namespaces:** `UBAG-AUTH-*`, `UBAG-VALIDATION-*`, `UBAG-QUOTA-*`, `UBAG-RATE-*`, `UBAG-QUEUE-*`, `UBAG-WORKER-*`, `UBAG-BROWSER-*`, `UBAG-CONTEXT-*`, `UBAG-TAB-*`, `UBAG-CONCURRENCY-*`, `UBAG-ADAPTER-*`, `UBAG-TARGET-*`, `UBAG-TEMPLATE-*`, `UBAG-CACHE-*`, `UBAG-WEBHOOK-*`, `UBAG-INTERNAL-*`.

Representative codes for the v2.1 multi-tab layer (all carry `retryable` + `retry_after_ms`):

| Code | Meaning | Retryable |
|---|---|---|
| `UBAG-CONCURRENCY-001` | Per-provider concurrency ceiling reached; job parked in pool queue | yes (auto) |
| `UBAG-CONCURRENCY-PROVIDER-THROTTLED-007` | Target signalled throttle/429; AIMD cap reduced, pacer widened | yes |
| `UBAG-CONTEXT-LOGIN-EXPIRED-002` | Provider context logged out; single-flight re-login in progress | yes |
| `UBAG-CONTEXT-SPA-SINGLETON-011` | Target supports only one live conversation per login; fanned out to multi-context | informational |
| `UBAG-TAB-CRASHED-003` | Channel tab crashed; job requeued, replacement tab opened | yes |
| `UBAG-TAB-MEMORY-RECYCLED-009` | Tab exceeded RSS budget and was recycled mid-idle | yes |
...
> **v2.1 concurrency model — read this first.** A "browser" is no longer one chat in one window. UBAG drives a **three-level hierarchy** so that a single browser can hold many logged-in providers at once, and each provider can fan out into many parallel tabs:
>
> ```
> Worker process
> └─ Browser instance          ← 1 Chromium/Firefox/WebKit process   (resource + crash domain)
>    ├─ Provider context A      ← DeepSeek login   (cookies · storage · fingerprint · proxy)
>    │   ├─ Tab A1             ← conversation / parallel channel #1
>    │   ├─ Tab A2             ← conversation / parallel channel #2   (auto-opened on demand)
>    │   └─ Tab A3             ← …
>    ├─ Provider context B      ← Claude.ai login  (independent identity & fingerprint)
>    │   ├─ Tab B1
>    │   └─ Tab B2
>    └─ Provider context C      ← ChatGPT login
>        └─ Tab C1
> ```
>
> | Your mental model | UBAG primitive | Isolates | Holds | Concurrency role |
> |---|---|---|---|---|
> | "a browser" | **Browser instance** (Playwright `Browser`) | OS process | many contexts | resource & crash domain |
> | "logged into provider X" | **Provider context** (`BrowserContext`) | cookies, storage, fingerprint, proxy | one login / identity | identity & rate-limit domain |
> | "a tab" / "a parallel channel" | **Channel tab** (`Page`) | nothing (shares its context) | one live conversation | the unit of in-flight work |
>
> This supersedes the one-job-per-session simplification in v2.0. Sections **12.1–12.5** below describe the lifecycle of a **provider context** (still called a "session" in the DB and some tables for backwards compatibility); **tabs** are the new sub-unit layered on top, and **12.6–12.13** define how they are pooled, scaled, paced, isolated, and recovered.
...
### 12.6 Concurrency model (v2.1 — tab-parallel)
The model is governed by five invariants. Every PR touching the worker honors them.

- **INV-1 — One conversation per tab.** A tab runs exactly one conversation at a time, so the v2.0 guarantee (no two jobs racing inside one chat thread) is preserved *per tab* while the fleet runs massively parallel *across tabs*.
- **INV-2 — Same provider + identity ⇒ shared login, new tab.** Additional requests for a provider the worker is already logged into open a **new tab in the existing authenticated context**. No re-login, no cold start — warm in ~200–600 ms because cookies/storage are already present.
- **INV-3 — Different provider (or identity) ⇒ new context, same browser.** Providers co-reside as separate tab groups in one browser instance: isolated cookies, storage, fingerprint, and proxy, but one shared process for efficiency.
- **INV-4 — Provider patience is a managed resource.** Per (provider, identity) there is an *adaptive* ceiling on concurrent tabs plus a global submit pacer (§12.9). UBAG opens as many tabs as a provider quietly tolerates and backs off the instant it complains. Opening tabs without this is the #1 cause of CAPTCHAs and account bans.
- **INV-5 — Tenant isolation is absolute.** Contexts and tabs are **never** shared across tenants. A multi-provider browser belongs to exactly one tenant (Tier 3+ pins browsers to tenants; edge/small are single-tenant by definition). Efficiency never overrides isolation.

Capacity guidance (commodity 8-core / 16 GB worker): ~3–6 browser instances, ~6–12 provider contexts, ~30–60 concurrent tabs total — throttled by the memory budget (§12.11) and per-provider caps (§12.9), not by a fixed magic number. Multiple workers still run per host (§ unchanged: ~1 worker / 2 cores for browser-heavy work).

### 12.7 Multi-provider browser (many providers, one browser)
A single browser instance hosts **one provider context per (provider, identity)** the tenant is logged into. This is exactly the "tab 1 = provider A, tab 2 = provider B, tab 3 = provider C" workflow, made safe:

- Each context carries its own `storage_state` (cookies + origin localStorage/IndexedDB), fingerprint id, locale/timezone, and optional proxy — so providers cannot see each other's cookies and detection signals stay per-identity consistent.
- Contexts are created **lazily** on the first job for that provider and torn down on idle TTL.
- A browser instance has a **context ceiling** (default 6) and a **tab ceiling** (default 12). Exceeding either spills new contexts onto another browser instance (or triggers a burst spawn).
- The same provider under **two different logins** (e.g. two DeepSeek accounts) is two contexts, optionally on two browsers, each with its own proxy — useful for higher aggregate throughput without over-driving a single account.

### 12.8 Parallel-channel tab pools (same provider, many tabs, auto-scaled)
Each (tenant, provider, identity) owns a **ChannelPool** — the component that turns *"more than one request for the same provider"* into *"open another tab and run it in parallel, then return the result."*

Per-pool state: `min_tabs`, `max_tabs`, `idle_ttl`, a bounded priority `pool_queue`, the live tabs and their states (`warming | ready | busy | draining | quarantined`), and the adaptive controller (§12.9).

Assignment algorithm — executed on every enqueue **and** every tab-free event:

```
on job J for pool P:
  if J.conversation_id is pinned to tab T:            # INV-1 sticky routing (multi-turn)
        if T.ready: assign(J, T)
        else:       enqueue(J, behind=T)              # same conversation cannot parallelize
  elif exists ready idle tab T in P:
        assign(J, T)                                  # reuse — fastest path
  elif len(P.tabs) < P.effective_max and P.tokens.allow():
        T = P.context.new_tab(); warm(T); assign(J, T)   # AUTO-OPEN a new parallel tab
  else:
        enqueue(J, P.pool_queue)                      # backpressure; queue is never unbounded

# effective_max = min(config.max_tabs, provider_cap_from_AIMD, memory_budget_tabs)
```

- **Warm pools** (`min_tabs ≥ 1`) keep p95 low for bursty providers — a ready tab is already on the chat route.
- **Scale-down:** tabs idle past `idle_ttl` close down to `min_tabs`.
- **Recycle:** a tab is drained and replaced after N jobs, M minutes, or RSS over budget — keeping memory clean and rotating per-tab state.
- The "open a new tab" path is cheap **precisely because of INV-2**: the authenticated context is reused, so a new channel skips login entirely.

### 12.9 Adaptive concurrency & anti-ban guardrails
Naively maximizing tabs gets accounts flagged. UBAG treats each provider+identity as a system with unknown, changing patience and controls it with a feedback loop:

- **AIMD controller.** Start at a conservative `max_tabs` (e.g. 2). After a sustained success window, *additively increase* the cap by 1. On any negative signal, *multiplicatively cut* it (e.g. ×0.5) and enter cooldown.
- **Negative signals (cut + cooldown):** HTTP 429 / soft "slow down" banners, CAPTCHA appearance, unexpected logout, response-throttle keywords, p95 latency past a guard, or an error-rate spike. The adapter detects these and reports them as structured signals.
- **Global submit pacer.** A per-(provider, identity) token bucket spaces prompt submissions across *all* of that provider's tabs with a jittered minimum gap (e.g. 800 ms ± 400 ms), so six tabs never fire simultaneously. Pacing is tunable per adapter.
- **Conversation-model awareness (correctness gate).** Adapters declare how the target handles parallel chats:
  - `url` / `tabbed` — a new conversation is a new route/URL (e.g. `…/chat/<id>`). **Safe for shared-context multi-tab; the default fast path.**
  - `spa-singleton` — the UI supports only one active conversation per login. **Shared-context multi-tab is disabled**; the pool transparently fans out into **multi-context** instead (one context per channel), trading the warm-login speedup for correctness. UBAG never runs two parallel chats in a session that cannot support them.
- **Observable backoff.** Every cap change emits a metric + audit event; operators see live per-provider concurrency and the current AIMD ceiling in the dashboard.

### 12.10 Shared-context race safety
Tabs in one context share cookies and origin storage, so concurrent operations must be made safe:

- **Single-flight (re-)login.** Each context owns an `auth_mutex`. The first tab to observe `logged_out` acquires it, **parks** sibling tabs, performs **one** re-login (handling 2FA via the live-login flow in §12.5 when needed), refreshes `storage_state`, then releases — siblings resume. N tabs never trigger N logins.
- **Per-tab conversation isolation.** Tabs create/resume conversations by route (`page.goto` the new-chat URL), never by clicking a shared singleton control that mutates global UI state.
- **Storage discipline.** Adapters must not write shared `localStorage` keys mid-job; targets that key conversation state by id (IndexedDB) are parallel-safe and are detected as such.
- **Snapshot on the safe side.** `storage_state` is snapshotted per context every N minutes and on graceful events, enabling sub-second context restore after a crash (§12.12).

### 12.11 Tab lifecycle, health & memory governance
- **States:** `warming → ready → busy → (ready | draining) ; → quarantined ; → closed`.
- **Health probe** per tab between jobs: DOM responsive, expected chat affordance present, no error/CAPTCHA wall. Failing tabs are quarantined and replaced.
- **Memory budget.** Hard per-tab RSS cap (default 250 MB) and a per-browser cap; an **LRU reaper** closes the least-recently-used *idle* tab when a browser nears its budget. DOM-node and listener thresholds trigger proactive recycle (extends v2.0 §19.3).
- **Zombie reaping.** A watchdog closes tabs with no heartbeat, stuck navigations, or orphaned state; their jobs requeue idempotently.

### 12.12 Failure isolation (blast radius by level)
The hierarchy is a set of nested bulkheads (ties into §20.3): the smaller the failure, the smaller and faster the recovery.

| Crash level | Blast radius | Automatic recovery |
|---|---|---|
| **Tab** (`Page`) | one conversation | reap tab → requeue job (idempotent resume) → open replacement tab |
| **Provider context** | all tabs of one provider+identity | recreate context → restore `storage_state` snapshot → re-warm `min_tabs` → requeue in-flight |
| **Browser instance** | all providers' tabs on that browser | respawn browser → recreate contexts from snapshots → requeue; sibling browsers untouched |
| **Worker** | all browsers on that worker | transactional outbox + queue requeue everything → orchestrator reassigns to another worker |

### 12.13 Scheduling & fairness across tabs
- **Work-stealing within a pool:** any ready tab pulls the head of its `pool_queue`.
- **Weighted scheduling across pools on a browser:** respects the 5 priority lanes (§14.4) and the per-provider concurrency tokens (§12.9); a `bulk`-lane provider can't starve a `critical` job on another provider.
- **Anti-starvation:** queued jobs gain priority with age.
- **Sticky multi-turn:** a follow-up carrying a `conversation_id` is pinned to the owning tab (INV-1); it queues *behind* that tab rather than opening a parallel tab, preserving chat memory.
...
- Per-worker hard limits: max RSS, max CPU, max open contexts, **max open tabs**, max session lifetime.
...
### 13.10 Pluggable browser engine (cross-engine, future-proof)
The worker abstracts the driver behind an `Engine` interface so the **same adapter logic** runs on multiple browsers and protocols:

| Engine | Protocol | Use |
|---|---|---|
| **Chromium + Patchright** (default) | CDP | Best stealth + ergonomics today |
| **Firefox** | Playwright / **WebDriver BiDi** | Second-engine diversity, different fingerprint surface |
| **WebKit** | Playwright | Safari-like rendering coverage |
| **Any W3C browser** | **WebDriver BiDi** | Vendor-neutral, standards-based, future-proof |

**WebDriver BiDi** (the W3C bidirectional standard) is a first-class target, so UBAG is not locked to CDP/Chromium long-term. Engine is selectable per target/adapter: a provider that starts detecting one engine can be moved to another by **config only**, no code change — a strong drift/anti-bot lever and a cross-compatibility win.

### 13.11 Remote browser grids (bring-your-own farm)
Workers can drive **remote** browsers over CDP/BiDi instead of (or alongside) local ones:
- `playwright.connect()` to a **Playwright server**, **browserless**, or a self-hosted grid.
- **Selenium Grid 4** and Docker `seleniarm` images (ARM64) via BiDi.
- Keeps the control plane tiny while browser capacity scales horizontally and independently — and lets browsers live in a hardened, network-isolated subnet separate from the gateway.

### 13.12 Engine-portable selectors
Selector strategies (§13.4) prefer **accessibility-role / ARIA** locators and **BiDi locators** that behave identically across Chromium/Firefox/WebKit, with `test-id` and CSS as engine-specific fast paths and the ML vision model as last resort. This both hardens against UI drift and keeps adapters portable across engines.
...
- Pre-built dashboards shipped in `infra/grafana/dashboards/`:
  ...
  - Channel-pool concurrency: tabs per provider, per-provider AIMD ceiling, submit-pacer wait, queue depth per pool, tab recycle/quarantine rate
...
| Open new parallel tab in a warm context (INV-2 fast path) | 300–600 ms |
| Provider contexts per browser instance (typical) | 6 |
| Concurrent channel tabs per browser instance (typical) | 12 |
| Concurrent channel tabs per worker (8-core / 16 GB) | 30–60 |
| AIMD reaction time to a 429 (cut + pacer widen) | < 1 request cycle |
...
| Provider rate-limit / ban risk | 429 / throttle banner / latency guard | AIMD cut + widen submit pacer + cooldown (§12.9) |
| Tab crash | Page `crash` event + heartbeat gap | Reap tab, requeue job (idempotent), open replacement (§12.12) |
| Context crash / login expiry across tabs | Context `close` / `logged_out` detect | Single-flight re-login or restore `storage_state` snapshot; park then resume sibling tabs (§12.10) |
| Zombie / leaked tab | No heartbeat, stuck nav, RSS over budget | Watchdog + LRU reaper close & requeue (§12.11) |
| spa-singleton target under parallel load | Adapter declares conversation_model | Fan out to multi-context instead of shared-context tabs (§12.9) |
...
-- Sessions  (v2.0 table; in v2.1 this is the compatibility view of provider_contexts)
...
-- v2.1 multi-tab model: browser instances → provider contexts → channel tabs
CREATE TABLE browser_instances (
  id              BIGSERIAL PRIMARY KEY,
  instance_id     TEXT UNIQUE NOT NULL,
  worker_id       TEXT NOT NULL,
  tenant_id       BIGINT NOT NULL REFERENCES tenants(id),
  engine          TEXT NOT NULL DEFAULT 'chromium',   -- chromium | firefox | webkit | bidi
  remote_endpoint TEXT,                                -- NULL = local; else grid/CDP/BiDi URL
...
CREATE INDEX ON browser_instances (tenant_id, state);

CREATE TABLE provider_contexts (
  id                 BIGSERIAL PRIMARY KEY,
  context_id         TEXT UNIQUE NOT NULL,
  instance_id        TEXT NOT NULL REFERENCES browser_instances(instance_id) ON DELETE CASCADE,
  tenant_id          BIGINT NOT NULL REFERENCES tenants(id),
  target_id          BIGINT NOT NULL REFERENCES targets(id),
  identity_ref       TEXT NOT NULL,                    -- which login / account
  login_state        TEXT NOT NULL DEFAULT 'unknown',
  conversation_model TEXT NOT NULL DEFAULT 'url',      -- url | tabbed | spa-singleton
  fingerprint_id     TEXT,
  proxy_id           TEXT,
  storage_state_uri  TEXT,                             -- object-store snapshot location
  max_tabs           INT  NOT NULL DEFAULT 2,          -- current AIMD ceiling
  created_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
  last_health_at     TIMESTAMPTZ,
  recycle_at         TIMESTAMPTZ,
  UNIQUE (tenant_id, target_id, identity_ref)
);
CREATE INDEX ON provider_contexts (instance_id);
...
CREATE TABLE browser_tabs (
  ...
  state           TEXT NOT NULL DEFAULT 'warming',     -- warming|ready|busy|draining|quarantined
  conversation_id TEXT,                                -- pinned conversation (sticky routing)
  current_job_id  TEXT,
  jobs_completed  INT  NOT NULL DEFAULT 0,
  rss_bytes       BIGINT,
  last_health_at  TIMESTAMPTZ,
...
CREATE INDEX ON browser_tabs (context_id, state);
CREATE INDEX ON browser_tabs (conversation_id) WHERE conversation_id IS NOT NULL;
...
### Alert: `UBAG.ProviderConcurrencyThrottled`
1. Dashboard → Channel Pools → identify the (provider, identity) whose AIMD ceiling collapsed.
2. Confirm the trigger: 429s, throttle banner, CAPTCHA, or latency guard.
3. Verify the pacer widened and the cap is recovering on the success window; do **not** manually raise `max_tabs` while in cooldown.
4. If chronic, lower that provider's `max_tabs` baseline, add a second identity/proxy, or shift to multi-context fan-out.
...
- ✅ **Multi-provider browser** — many logged-in providers as tab groups in one browser (§12.7)
- ✅ **Parallel-channel tab pools** — auto-open tabs for the same provider on demand (§12.8)
- ✅ **Adaptive per-provider concurrency (AIMD) + global submit pacer** — anti-ban (§12.9)
- ✅ **Conversation-model detection** (url / tabbed / spa-singleton) with multi-context fallback (§12.9)
- ✅ **Single-flight re-login** + per-tab conversation isolation (§12.10)
- ✅ **Blast-radius failure isolation** (tab / context / browser / worker) + storage-state snapshots (§12.12)
- ✅ **Cross-engine**: Chromium (CDP), Firefox, WebKit, WebDriver BiDi (§13.10)
- ✅ **Remote browser grids**: browserless / Selenium Grid 4 / Playwright server (§13.11)
- ✅ **Engine-portable accessibility-role / BiDi selectors** (§13.12)
...
- **AIMD** — Additive-Increase / Multiplicative-Decrease controller that adapts per-provider tab concurrency to avoid bans.
- **App** — A client integration (e.g., "Radiology Reporting Windows").
- **Bulkhead** — Resource isolation between tenants/targets.
- **Browser instance** — One Chromium/Firefox/WebKit process; the resource and crash domain that can host many provider contexts.
- **Canary** — Small-percentage rollout of a new adapter version.
- **Channel tab** — One browser tab (`Page`) running exactly one conversation; the unit of in-flight parallel work.
- **ChannelPool** — Per (tenant, provider, identity) manager that opens/scales/recycles channel tabs and queues overflow.
- **Conversation model** — How a target supports parallel chats: `url`, `tabbed`, or `spa-singleton` (drives shared-context vs multi-context fan-out).
...
- **Provider context** — One `BrowserContext` per (provider, identity); holds the login, fingerprint, and proxy; the identity & rate-limit domain shared by its tabs.
- **Saga** — Multi-step workflow with compensations.
- **Sidecar (UBAG)** — Local-host Rust connector for legacy desktop apps.
- **Single-flight re-login** — One re-login per context while sibling tabs park, instead of N concurrent logins.
- **Storage-state snapshot** — Periodic cookies+origin-storage capture of a context for sub-second crash restore.
- **Submit pacer** — Per-provider token bucket that spaces prompt submissions across all of a provider's tabs.
...
- **WebDriver BiDi** — W3C bidirectional browser-automation standard; vendor-neutral engine option alongside CDP.
...
## Appendix C — Worked example: multi-tab, multi-provider orchestration

### C.1 Worker concurrency config (`worker.yaml`)

```yaml
worker:
  engine_default: chromium          # chromium | firefox | webkit | bidi
  remote_endpoints: []              # e.g. ["wss://browserless.internal:3000"] for a grid
  limits:
    max_browser_instances: 4
    max_contexts_per_browser: 6
    max_tabs_per_browser: 12
    max_tabs_total: 48
    tab_rss_budget_mb: 250

channel_pools:
  defaults:
    min_tabs: 1                     # warm tab kept ready
    max_tabs: 4                     # hard ceiling (AIMD may keep it lower)
    idle_ttl_seconds: 120
    recycle_after_jobs: 25
    submit_pacer: { min_gap_ms: 800, jitter_ms: 400 }
    aimd: { start: 2, increase_step: 1, decrease_factor: 0.5, cooldown_s: 60 }

  per_target:                       # overrides — tune to each provider's tolerance
    deepseek_web:   { max_tabs: 6, conversation_model: url }
    claude_web:     { max_tabs: 4, conversation_model: url }
    chatgpt_web:    { max_tabs: 3, conversation_model: url,         submit_pacer: { min_gap_ms: 1200, jitter_ms: 600 } }
    some_spa_chat:  { max_tabs: 4, conversation_model: spa-singleton }   # auto multi-context fan-out
```

### C.2 What happens at runtime

Three providers are needed at once, and DeepSeek receives a burst of six requests:

```
Worker-1
└─ Browser instance #1  (tenant: hospital-rad)
   ├─ Context: deepseek_web / acct-A     [conversation_model=url, AIMD cap=6]
   │   ├─ Tab D1  busy  (job 1001)        ← warm tab
   │   ├─ Tab D2  busy  (job 1002)        ← auto-opened (reused login, ~400 ms)
   │   ├─ Tab D3  busy  (job 1003)        ← auto-opened
   │   ├─ Tab D4  busy  (job 1004)
   │   └─ (jobs 1005, 1006 wait in pool_queue — cap reached; pacer spacing submits)
   ├─ Context: claude_web / acct-A        [logged in once, reused]
   │   └─ Tab C1  busy  (job 1007)
   └─ Context: chatgpt_web / acct-A
       └─ Tab G1  busy  (job 1008)
```

- Jobs 1001–1004 run **in parallel** in four DeepSeek tabs that all **share one login** (INV-2).
- Jobs 1005–1006 **wait** because DeepSeek's adaptive cap is 6 but the submit pacer is spacing them out; as D1 frees, 1005 is assigned to it (work-stealing).
- Claude and ChatGPT run **concurrently in the same browser** as isolated contexts (INV-3).
- If ChatGPT returns a 429, its AIMD cap drops to ~1 and its pacer widens — DeepSeek and Claude are unaffected (per-provider bulkhead).
- If Tab D2 crashes, only job 1002 requeues and a replacement tab opens; siblings keep running (§12.12).

### C.3 Client side — fire many jobs, they auto-parallelize

The client does nothing special: it submits jobs and the gateway + worker handle tab fan-out, pacing, and ordering.

```ts
import { Ubag } from "@ubag/sdk";
const ubag = new Ubag({ appId: "radiology-electron", appSecret: process.env.UBAG_SECRET! });

// Six DeepSeek requests at once → auto-spread across parallel tabs (capped + paced)
const ctScans = [/* …six findings… */];
const reports = await Promise.all(
  ctScans.map((findings) =>
    ubag.jobs.run({ target: "deepseek_web", templateId: "radiology_ct_brain_v3", input: { findings } })
  )
);

// Different providers in flight simultaneously — separate contexts, one browser
const [claudeSummary, gptTranslation] = await Promise.all([
  ubag.jobs.run({ target: "claude_web",  prompt: "Summarize this discharge letter…" }),
  ubag.jobs.run({ target: "chatgpt_web", prompt: "Translate to Arabic…" }),
]);

// Multi-turn stays on ONE tab (sticky) — pass conversation_id to preserve chat memory
const followUp = await ubag.jobs.run({
  target: "deepseek_web",
  conversationId: reports[0].conversation_id,   // pinned to the owning tab (INV-1)
  prompt: "Now add a comparison with the prior study.",
});
```

The platform turns "many requests to the same and different providers" into the right number of parallel tabs across the right contexts — safely, and without the client having to know any of it.
